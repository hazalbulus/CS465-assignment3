...
const ELLIPSOID = 0;
const HYPERBOLOID = 1;
const BREATHER = 2; // New constant for Breather surface mode
...
// Breather surface variables
var verticesB = [];
var indicesB = [];
var normalsB = [];
var loopSizeB = 0;
var aa = 0.5; // Example value, you can set it from UI
...
function drawObject() {
  if (objectMode === BREATHER) {
    processBuffers(vec4(0.0, 0.0, 0.0, 1.0), verticesB, normalsB, indicesB);
...
function generateEllipsoidVertices() {
  ...
  for (var u = -1.0; u < 1.0; u += 0.01) {
    for (var v = -1.0; v < 1.0; v += 0.01) {
      var x =
        a1 *
        Math.pow(Math.cos(u * (Math.PI / 2)), e1) *
        Math.pow(Math.cos(v * Math.PI), e2);
      var y =
        a2 *
        Math.pow(Math.cos(u * (Math.PI / 2)), e1) *
        Math.pow(Math.sin(v * Math.PI), e2);
  ...
  for (var u = -1.0; u < 1.0; u += 0.01) {
    for (var v = -1.0; v < 1.0; v += 0.01) {
      var x =
        1 /
        (a1 *
          Math.pow(Math.cos(u * (Math.PI / 2)), 2 - e1) *
          Math.pow(Math.cos(v * Math.PI), 2 - e2));
      var y =
        1 /
        (a2 *
          Math.pow(Math.cos(u * (Math.PI / 2)), 2 - e1) *
          Math.pow(Math.sin(v * Math.PI), 2 - e2));
...
function generateHyperboloidVertices() {
  ...
  for (var u = -1.0; u < 1.0; u += 0.01) {
    for (var v = -1.0; v < 1.0; v += 0.01) {
      var x =
        a1 *
        Math.pow(1 / Math.cos(u * Math.PI), e1) *
        Math.pow(Math.cos(v * Math.PI), e2);
      var y =
        a2 *
        Math.pow(1 / Math.cos(u * Math.PI), e1) *
...
function generateHyperboloidNormals() {
  ...
  for (var u = -1.0; u < 1.0; u += 0.01) {
    for (var v = -1.0; v < 1.0; v += 0.01) {
      var x =
        (1 / a1) *
        Math.pow(1 / Math.cos(u * Math.PI), 2 - e1) *
        Math.pow(Math.cos(v * Math.PI), 2 - e2);
      var y =
        (1 / a2) *
        Math.pow(1 / Math.cos(u * Math.PI), 2 - e1) *
...
function generateBreatherVertices() {
  ...
  var u, v;
  var denom, x, y, z;
  var step = 0.1; // Define the step size for u and v
  var range = 15; // Define the range for u and v

  for (u = -range; u <= range; u += step) {
    for (v = -range; v <= range; v += step) {
      var w = Math.sqrt(1 - aa * aa);
      denom =
        aa *
        (Math.cosh(aa * u) * Math.cosh(aa * u) +
          aa * Math.sin(w * v) * Math.sin(w * v));
      x =
        -u +
        ((2 * (1 - aa * aa)) / denom) * Math.cosh(aa * u) * Math.sin(aa * u);
      y =
        ((2 * w * Math.cosh(aa * u)) / denom) *
        (-Math.cos(v) * Math.cos(w * v) - Math.sin(v) * Math.sin(w * v));
      z =
        ((2 * w * Math.cosh(aa * u)) / denom) *
        (-Math.sin(v) * Math.cos(w * v) + Math.cos(v) * Math.sin(w * v));

      verticesB.push(vec4(x, y, z, 1.0));
    }
  }
  loopSizeB = Math.floor((2 * range) / step) + 1; // Update loop size based on the number of vertices per row
}
...
function generateBreatherIndices() {
  ...
  // Assuming loopSizeB is the number of vertices per row/column
  for (var i = 0; i < loopSizeB - 1; i++) {
    for (var j = 0; j < loopSizeB; j++) {
      indicesB.push(i * loopSizeB + j);
      indicesB.push((i + 1) * loopSizeB + j);
    }
  }
}
...
function radios() {
  ...
  document.getElementById("Breather").onchange = function () {
    objectMode = BREATHER;
  };
...
function processBuffers(color, vertices, normals, indices) {
  ...
  for (var i = 0; i < vertices.length; i++) colors.push(color);
